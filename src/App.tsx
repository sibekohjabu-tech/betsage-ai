...
export default function App() {
  ...
  function afterCheckout(u: unknown) {
    setUser(u as User);
    setCheckout(null);
    setPage("dashboard");
  }
  ...
  function startTrial(plan: (typeof PLANS)[number]) {
    if (!user) {
    ...
    <>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        ::-webkit-scrollbar-thumb{background:#182840;border-radius:4px}
        ...
        @media (max-width: 768px) {
          ...
          .page-grid-4 { grid-template-columns: repeat(2, 1fr) !important; }
          .page-grid-5 { grid-template-columns: repeat(2, 1fr) !important; }
          .tracker-grid { grid-template-columns: repeat(3, 1fr) !important; }
      ...
      >
        <div style={{ maxWidth: 1320, margin: "0 auto", padding: "0 20px", width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(0,229,255,.07)", border: "1px solid rgba(0,229,255,.18)", borderRadius: 20, padding: "4px 12px", fontSize: 11, fontWeight: 700, color: G.accent }}>
              <Dot c={G.green} pulse />{TODAY}
            </div>
            <div style={{ background: "rgba(255,209,102,.08)", border: "1px solid rgba(255,209,102,.2)", borderRadius: 20, padding: "4px 12px", fontSize: 11, color: G.gold, fontWeight: 700 }}>
              🏆 WC2026 · {WC_DAYS}d
            </div>
            {!user ? (
              <>
        ...
        >
          {PAGES.map((p) => (
            <button
              ...
              style={{
                ...
                padding: "10px 12px",
                borderRadius: 10,
                borderLeft: page === p.key ? `2px solid ${G.accent}` : "2px solid transparent",
                background: page === p.key ? "rgba(0,229,255,.07)" : "transparent",
                color: page === p.key ? G.accent : G.dim,
                fontFamily: "inherit",
                fontSize: 13,
                fontWeight: 600,
                cursor: "pointer",
            ...
            >
              ...
              {p.label}
              {p.badge && (
                <span style={{ marginLeft: "auto", background: `${G.gold}18`, color: G.gold, fontSize: 9, fontWeight: 800, padding: "2px 6px", borderRadius: 4, letterSpacing: 0.5 }}>{p.badge}</span>
              )}
            </button>
          ))}
          ...
          <div style={{ marginTop: "auto", paddingTop: 16, display: "flex", flexDirection: "column", gap: 10 }}>
            {!user ? (
              ...
              <div style={{ background: "rgba(0,255,136,.06)", border: "1px solid rgba(0,255,136,.15)", borderRadius: 10, padding: "10px 12px" }}>
                <div style={{ fontSize: 10, color: G.green, fontWeight: 700, marginBottom: 4 }}>TRIAL ACTIVE</div>
                <div style={{ fontSize: 11, color: G.dim }}>7 days free · {user.plan?.name} plan</div>
              </div>
            )}
        ...
        <div className="main-content" style={{ overflowX: "hidden", paddingBottom: 80 }}>
          ...
          {page === "live" && (
            <div style={{ padding: "28px 24px", maxWidth: 1060 }}>
              <div style={{ fontSize: 28, fontWeight: 900, marginBottom: 6 }}>🔴 Live Football Data</div>
              <div style={{ color: G.dim, fontSize: 14, marginBottom: 20 }}>Real-time scores, odds & fixtures from 9 leagues worldwide</div>
              <LiveFixtures />
            </div>
          )}

  ...
  return (
    <div style={{ padding: "28px 24px", maxWidth: 1060 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 22, flexWrap: "wrap" }}>
        {([["fixtures", "⚽ Fixtures & Picks"], ["groups", "🗂️ Groups"], ["outrights", "🏆 Outrights"], ["tips", "💡 Tips"]] as const).map(([k, l]) => (
        ...
        <>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
            {["ALL", "A", "B", "C", "D", "E", "F", "G", "H"].map((g) => (
              <SBtn key={g} sm active={gf === g} onClick={() => setGf(g)}>{g === "ALL" ? "All" : g}</SBtn>
            ))}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: 14 }}>
            {filtered.map((f, i) => (
              <div key={i} className="fu" style={{ background: G.card2, border: `1px solid ${G.border}`, borderRadius: 12, padding: 18, animationDelay: `${i * 0.05}s` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                  <div>
                    <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
                      <span style={{ width: 24, height: 24, borderRadius: 6, background: "rgba(0,229,255,.12)", color: G.accent, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, fontFamily: "monospace" }}>{f.group}</span>
                      <span style={{ fontSize: 11, color: G.dim }}>{f.date} · {f.time}</span>
                    </div>
          ...
          {WC_OUTRIGHTS.map((o, i) => (
            <Card2 key={i}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                ...
                <span style={{ fontFamily: "monospace", fontWeight: 700, color: G.green }}>{o.prob}%</span>
              </div>
      ...
      {tab === "tips" && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 14 }}>
          {[{ icon: "⚽", title: "Over 2.5 Goals — WC Special", color: G.green, tip: "WC group stage averages 2.8 goals/match. Teams attack-minded as 3 points are crucial." }, { icon: "📐", title: "Corners — Big Teams Win Corners", color: G.purple, tip: "Top 10 teams generate 6+ corners vs defensive underdogs." }, { icon: "🟨", title: "Bookings — Pressure Cooker", color: G.red, tip: "WC group stage avg 3.8 cards. High pressure + mixed referee styles." }, { icon: "⚖️", title: "Handicap — Big Team Value", color: G.accent, tip: "Asian handicap -1.5 when top teams face rank 30+ opponents." }, { icon: "🤝", title: "Draw — Final Group Game Trap", color: G.gold, tip: "Final group game draws occur when one team is already through." }, { icon: "🎰", title: "WC Accumulators", color: G.orange, tip: "4-leg WC accas with O2.5 Goals = 8x–14x combined odds." }].map((t, i) => (

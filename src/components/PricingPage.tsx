...
export function PricingPage({ onSelect }: PricingPageProps) {
  ...
  return (
    <div style={{ padding: "28px 24px", maxWidth: 980 }}>
      <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(0,229,255,.07)", border: "1px solid rgba(0,229,255,.18)", borderRadius: 20, padding: "5px 14px", fontSize: 11, fontWeight: 700, color: G.accent, marginBottom: 16 }}>
        <Dot c={G.green} pulse />
        ...
        {PLANS.map((plan) => (
          ...
          >
            ...
            <div style={{ marginBottom: 10 }}>
              <span style={{ fontSize: 46, fontWeight: 700, color: plan.badge === "MOST POPULAR" ? G.green : G.text, fontFamily: "monospace" }}>${plan.price}</span>
              <span style={{ color: G.dim, fontSize: 14 }}>/mo</span>
            </div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(0,255,136,.08)", border: "1px solid rgba(0,255,136,.18)", borderRadius: 20, padding: "4px 12px", fontSize: 11, fontWeight: 700, color: G.green, marginBottom: 20 }}>
              🎁 7 days FREE
            </div>
            <div style={{ marginBottom: 24 }}>
              {plan.features.map((f, j) => (
                <div key={j} style={{ display: "flex", gap: 8, marginBottom: 9 }}>
                  ...
                  <span style={{ fontSize: 13, color: j < 2 ? G.text : G.dim, lineHeight: 1.4 }}>{f}</span>
                </div>
              ))}
            </div>
            <button
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
          {[
            ...
            <div key={r.plan} style={{ background: G.bg, borderRadius: 12, padding: 20 }}>
              ...
              <div style={{ fontWeight: 800, fontSize: 18 }}>{r.mrr} MRR</div>
            </div>
          ))}
        </div>
      </Card>

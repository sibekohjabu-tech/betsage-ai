      ...
      >
        ...
        <div style={{ background: "rgba(0,255,136,.05)", border: "1px solid rgba(0,255,136,.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 18 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, textAlign: "center" }}>
            {([
              ["Today", "FREE ✓", G.green],
              [`Day 7`, ted.split(" ").slice(0, 2).join(" "), G.gold],
              ["Day 8+", `$${plan.price}/mo`, G.text],
            ] as const).map(([d, v, c]) => (
              <div key={d}>
        ...
        {msg && (
          <div style={{ padding: "10px 14px", background: "rgba(255,69,96,.1)", border: "1px solid rgba(255,69,96,.2)", borderRadius: 8, fontSize: 13, color: G.red, marginBottom: 12 }}>
            {msg}
          </div>
        )}

...
interface CheckoutModalProps { plan: Plan; user: { name?: string; email?: string } | null; onClose: () => void; onSuccess: (user: Record<string, unknown>) => void; }
export function CheckoutModal({ plan, user, onClose, onSuccess }: CheckoutModalProps) {
  ...
  const inp: React.CSSProperties = { width: "100%", background: "#070E1A", border: `1px solid ${G.border}`, borderRadius: 9, padding: "12px 14px", color: G.text, fontFamily: "monospace", fontSize: 14, outline: "none" };
  function fc(v: string) { return v.replace(/\D/g, "").replace(/(.{4})/g, "$1 ").trim().slice(0, 19); }
  function fe(v: string) { return v.replace(/\D/g, "").replace(/^(\d{2})(\d)/, "$1/$2").slice(0, 5); }
  async function pay() {
    ...
    setLoading(true); setMsg(null); await new Promise((r) => setTimeout(r, 2400));
    const u = { ...user, plan, trial: true, subscribed: true };
    localStorage.setItem("ea_user", JSON.stringify(u)); onSuccess(u);
  }
  return (
      ...
      <div className="pop" style={{ background: G.card, border: `1px solid ${G.border}`, borderRadius: 20, width: "100%", maxWidth: 420, padding: 32, position: "relative" }}>
        ...
        <div style={{ background: "rgba(0,255,136,.05)", border: "1px solid rgba(0,255,136,.15)", borderRadius: 12, padding: "14px 18px", marginBottom: 18 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, textAlign: "center" }}>
            {[ ["Today", "FREE ✓", G.green], ["Day 7", ted.split(" ").slice(0, 2).join(" "), G.gold], ["Day 8+", `$${plan.price}/mo`, G.text] ].map(([d, v, c]) => (<div key={d}><div style={{ fontFamily: "monospace", fontWeight: 700, fontSize: 14, color: c }}>{v}</div><div style={{ fontSize: 11, color: G.dim, marginTop: 3 }}>{d}</div></div>))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 12 }}><div style={{ fontSize: 11, color: G.dim, fontWeight: 700, marginBottom: 6 }}>CARD NUMBER</div><input style={inp} placeholder="1234 5678 9012 3456" value={card} onChange={(e) => setCard(fc(e.target.value))} maxLength={19} /></div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 14 }}><div><div style={{ fontSize: 11, color: G.dim, fontWeight: 700, marginBottom: 6 }}>EXPIRY</div><input style={inp} placeholder="MM/YY" value={exp} onChange={(e) => setExp(fe(e.target.value))} maxLength={5} /></div><div><div style={{ fontSize: 11, color: G.dim, fontWeight: 700, marginBottom: 6 }}>CVC</div><input style={inp} placeholder="123" value={cvc} onChange={(e) => setCvc(e.target.value.replace(/\D/g, "").slice(0, 4))} maxLength={4} /></div></div>
        ...
        <button onClick={pay} disabled={loading} style={{ width: "100%", padding: 15, background: G.gradGreen, color: "#000", fontFamily: "inherit", fontWeight: 700, borderRadius: 9, border: "none", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.65 : 1, fontSize: 15 }}>{loading ? "⟳ Activating trial..." : "Start Free Trial — No Charge Today 🚀"}</button>
